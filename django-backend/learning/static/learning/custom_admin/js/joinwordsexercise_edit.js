...
function addForm() {
    ...
    const firstInput = newForm.children[1].children[0]
    const secondInput = newForm.children[2].children[0]
    ...
    updateInputsPrefix(newCounter0, firstInput, secondInput, idInput, exerciseInput)

    bindNewInputs(firstInput, secondInput)
...
function removeForm(e) {
    ...
    for (let i = extraIdx + 1; i < extraFormNodes.length; i++) {
        ...
        const firstInput = extraForm.children[1].children[0]
        const secondInput = extraForm.children[2].children[0]
        const idInput = extraForm.children[4]
        const exerciseInput = extraForm.children[5]

        const counter0 = num - 2
        updateInputsCounter(counter0, firstInput, secondInput, idInput, exerciseInput)
    }
